...
public class CanyonDelay extends AFilter implements IConfigurable {
...
ArrayList<EffectParam> _defaultparams;
  ...
  public Float[] getConf() {
    return new Float[] { 
      ltr_time, rtl_time, ltr_feedback, rtl_feedback, cutoff
    };
  }

  public void setConf(Float[] conf) { 
    if ( conf.length < 5 ) throw new Error("setConf wrong param count, expected "+5+" got "+conf.length);
    ltr_time = conf[0];
    rtl_time = conf[1];
    ltr_feedback = conf[2];
    rtl_feedback = conf[3];
